...
All code for generating the data and running the benchmarks can be found in [https://github.com/developmentseed/tile-benchmarking](https://github.com/developmentseed/tile-benchmarking).
...
The focus of the code benchmarks is to understand how to optimize data and code for the best tiling performance. The authors replicated the tiling code to diagnose slowness due to data storage formatting and code libraries, without the introduction of a network.
...
To profile the code for rendering tiles with both XarrayReader and titiler-pgstac, code was copied from those projects as needed to inject timers.
...
A variety of datasets was selected for end-to-end testing and hopefully the framework makes it easy to modify and test new datasets and use cases come up. See the [tile-server-e2e-benchmarks.ipynb](./tile-server-e2e-benchmarks.ipynb) for specific datasets.
